...
OrchestrateConnector.prototype.create = function (model, data, callback) {
    ...
    create.then(function (response) {
        ...
        return callback && callback(null, self.tools.extractKey(response));
    }).fail(function (err) {
        if (self.debug) {
...
OrchestrateConnector.prototype.destroy = function destroy(model, id, callback) {
    ...
    if (self.db) {
        self.db.remove(model, id, true)
            .then(function (response) {
                if (self.debug) {
                    debug('destroy.success', model, id, response && response.statusCode ? response.statusCode : response ? response : 'no response??');
                }
                return callback && callback(null, response);
            })
            .fail(function (error) {
                if (self.debug) {
                    debug('destroy.failure', model, id, error);
                }
                callback && callback(error);
            });
    } else {
...
OrchestrateConnector.prototype.buildWhere = require('./build-where');
...
OrchestrateConnector.prototype.updateAttributes = function updateAttrs(model, id, data, callback) {
    ...
    if (!self.db) {
        var error = new Error('No orchestrate.io connection');
        callback && callback(error);
        throw error;
    } else {
        self.db.merge(model, id, data)
            .then(function (response) {
                if (self.debug) {
                    debug('updateAttributes.callback', model, id, response)
                }
